...
def check_callable(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
...
def check_dict(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    if not isinstance(value, dict):
        raise TypeCheckError('value is not a dict')

    if annotation is not dict:
        if (hasattr(annotation, "__args__") and
                annotation.__args__ not in (None, annotation.__parameters__)):
...
def check_typed_dict(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    declared_keys = frozenset(annotation.__annotations__)
    if hasattr(annotation, '__required_keys__'):
        required_keys = annotation.__required_keys__
    else:  # py3.8 and lower
        required_keys = declared_keys if annotation.__total__ else frozenset()
    ...
    for key, argtype in get_type_hints(annotation).items():
        argvalue = value.get(key, _missing)
        if argvalue is not _missing:
            check_type_internal(argvalue, argtype, memo)


def check_list(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    if not isinstance(value, list):
        raise TypeCheckError('is not a list')

    if annotation is not list:
        args = get_args(annotation)
        if args != annotation.__parameters__:
            value_type = args[0]
...
def check_sequence(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
...
def check_set(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    if not isinstance(value, AbstractSet):
        raise TypeCheckError('is not a set')

    if annotation is not set:
        if hasattr(annotation, "__args__") and annotation.__args__ not in \
                (None, annotation.__parameters__):
...
def check_tuple(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    # Specialized check for NamedTuples
    if sys.version_info >= (3, 8):
        is_named_tuple = hasattr(annotation, '__annotations__')
    else:
        is_named_tuple = hasattr(annotation, '_field_types')  # deprecated since python 3.8

    if is_named_tuple:
        if not isinstance(value, annotation):
            raise TypeCheckError(f'is not a named tuple of type {qualified_name(annotation)}')

        if sys.version_info >= (3, 8):
            field_types = annotation.__annotations__
        else:
            field_types = annotation._field_types
...
def check_union(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
...
def check_class(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    if not isclass(value):
        raise TypeCheckError('is not a class')

    # Needed on Python 3.7+
    if annotation is Type or annotation is type:
        return

    expected_class = get_args(annotation)[0]
...
def check_newtype(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
...
def check_instance(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    origin_type = get_origin(annotation) or annotation
    if not isinstance(value, origin_type):
        raise TypeCheckError(f'is not an instance of {qualified_name(origin_type)}')


def check_typevar(value: Any, typevar: TypeVar, memo: TypeCheckMemo, *,
                  subclass_check: bool = False) -> None:
    if typevar.__bound__ is not None:
        annotation = Type[typevar.__bound__] if subclass_check else typevar.__bound__
...
def check_literal(value: Any, expected_type: Any, memo: TypeCheckMemo) -> None:
    def get_literal_args(literal):
        retval = []
        for arg in get_args(literal):
            if get_origin(arg) is Literal:
                # The first check works on py3.6 and lower, the second one on py3.7+
                retval.extend(get_literal_args(arg))
    ...
    final_args = tuple(get_literal_args(expected_type))
...
def check_number(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    if annotation is complex and not isinstance(value, (complex, float, int)):
        raise TypeCheckError('is neither complex, float or int')
    elif annotation is float and not isinstance(value, (float, int)):
        raise TypeCheckError('is neither float or int')


def check_io(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    origin_type = get_origin(annotation) or annotation
    args = get_args(annotation)
...
def check_protocol(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
    # TODO: implement proper compatibility checking and support non-runtime protocols
    if getattr(annotation, '_is_runtime_protocol', False):
        if not isinstance(value, annotation):
            raise TypeCheckError(f'is not compatible with the {annotation.__qualname__} protocol')


def check_byteslike(value: Any, annotation: Any, memo: TypeCheckMemo) -> None:
